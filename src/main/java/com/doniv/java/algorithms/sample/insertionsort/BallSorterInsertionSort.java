package com.doniv.java.algorithms.sample.insertionsort;
...
public class BallSorterInsertionSort implements AlgorithmDemo {
    /**
    ...
    @Override
    public void demo() {
        System.out.println("Insertion Sort Sort Algorithm to sort a set of Balls in an Array.");
        ...
        baskets = returnSortedBallsWithInsertionSort(baskets, SortOrder.DESCENDING);
        System.out.println("Sorted Balls In Descending Order Using Insertion Sort");
        for (int i : baskets) {
            System.out.println(i);
        }

        baskets = returnSortedBallsWithInsertionSort(baskets, SortOrder.ASCENDING);
        System.out.println("Sorted Balls In AScending Order Using Insertion Sort");
        for (int i : baskets) {
            System.out.println(i);
        }
    }
     ...
     */
    public int[] returnSortedBallsWithInsertionSort(int[] baskets, SortOrder sortOrder) {
        int idx = 0;
        List<Integer> basketsList = Arrays.stream(baskets).boxed().collect(Collectors.toList());
        for (int sortedIndexPosition = 1; sortedIndexPosition <basketsList.size();  sortedIndexPosition++) {
            for(int k =sortedIndexPosition;k>0;k--) {
                if(sortOrder.name().equals(SortOrder.ASCENDING.name()) && basketsList.get(k-1) >  basketsList.get(k)) {
                    Collections.swap(basketsList,k,k-1);
                }  else if(sortOrder.name().equals(SortOrder.DESCENDING.name()) && basketsList.get(k-1) <  basketsList.get(k)) { 
                    Collections.swap(basketsList,k,k-1);                    
                }    
            }    
        }
